	...
	selectedBulbs.each { dni ->
		...
		if(!d) {
			...
			if (bulbs instanceof java.util.Map) {
				...
				d = addChildDevice("kelchm", "Hue Bulb", dni, newHueBulb?.hub, ["label":newHueBulb?.name])
			}
	...
	selectedGroups.each { dni ->
		...
		if(!d) 
		{
			...
			if (groups instanceof java.util.Map) 
			{
				...
				d = addChildDevice("kelchm", "Hue Group", dni, newHueGroup?.value.hub, ["label":newHueGroup?.value.name])
			} 
...
def addBridge() {
	...
	if(vbridge) {
		...
		if(!d) {
			d = addChildDevice("kelchm", "Hue Bridge", selectedHue, vbridge.value.hub, ["data":["mac": vbridge.value.mac]]) // ["preferences":["ip": vbridge.value.ip, "port":vbridge.value.port, "path":vbridge.value.ssdpPath, "term":vbridge.value.ssdpTerm]]
